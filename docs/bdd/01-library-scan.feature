Feature: 收藏來源與重新掃描
  ...
  Scenario: 將圖片資料夾視為一筆收藏
    Given 某個資料夾不是掃描來源本身
    And 該資料夾的直接子檔案含有支援的圖片檔案
    And 該資料夾的直接子檔案不含 ZIP 檔案
    When 系統掃描該資料夾
    Then 該資料夾本身應成為一筆收藏
    And 系統不應再進入其子資料夾，即使更深層存在 ZIP 檔案
    And 系統不應把其子資料夾各自當成另一筆收藏

  @current @scan-012
  Scenario: 同層同時含圖片與 ZIP 的資料夾不成為收藏
    Given 某個資料夾的直接子檔案同時含有圖片與 ZIP 檔案
    When 系統掃描該資料夾
    Then 該資料夾不應成為收藏
    And 其中的 ZIP 與子資料夾應照常掃描

  @current @scan-013
  Scenario: 掃描來源本身永遠只是容器
    Given 某個掃描來源的直接子檔案含有支援的圖片檔案
    When 系統掃描該掃描來源
    Then 該掃描來源本身不應成為收藏
    And 系統應繼續掃描其子項
  ...
  @current @scan-014
  Scenario: 只含變體子資料夾的父層視為一筆收藏
    Given 某個資料夾沒有直接圖片與 ZIP
    And 它有 2 到 4 個直接子資料夾
    And 每個子資料夾都只含圖片、沒有 ZIP 與更深的子資料夾
    And 子資料夾名稱不含方括號或圓括號
    And 子資料夾都尚未在收藏索引中
    When 系統掃描該資料夾
    Then 該資料夾本身應成為一筆收藏
