    ...
    R: RecycleBin + Send + 'static,
{
    ...
    Router::new()
        ...
        .route(
            "/api/export-roots",
            get(list_export_roots::<R>).post(register_export_root::<R>),
        )
        .route(
            "/api/export-roots/{root_id}",
            patch(update_export_root::<R>).delete(deactivate_export_root::<R>),
        )
        .route(
            "/api/export-roots/{root_id}/activate",
            post(reactivate_export_root::<R>),
        )
        .route("/api/export-jobs/preflight", post(preflight_export::<R>))
        .route("/api/export-jobs", post(create_export::<R>))
        .route("/api/export-jobs/current", get(get_current_export::<R>))
        .route("/api/export-jobs/{job_id}", get(get_export::<R>))
        .route("/api/export-jobs/{job_id}/retry", post(retry_export::<R>))
        .route(
...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct ExportRootRequest {
    path: String,
    label: String,
}

#[derive(Debug, Serialize)]
struct ExportRootsResponse {
    roots: Vec<ExportRootResponse>,
}

#[derive(Debug, Serialize)]
struct ExportRootResponse {
    id: i64,
    path: String,
    label: String,
    active: bool,
...
#[derive(Debug, Serialize)]
struct OpenExportLocationResponse {
    job_id: i64,
    launched: bool,
}
...
async fn register_export_root<R>(
    State(state): State<HttpState<R>>,
...
async fn deactivate_export_root<R>(
    State(state): State<HttpState<R>>,
    Path(root_id): Path<String>,
) -> Result<Json<ExportRootResponse>, ApiError>
where
...
async fn reactivate_export_root<R>(
    State(state): State<HttpState<R>>,
    Path(root_id): Path<String>,
) -> Result<Json<ExportRootResponse>, ApiError>
where
...
async fn set_export_root_active<R>(
    state: HttpState<R>,
    root_id: String,
    active: bool,
) -> Result<Json<ExportRootResponse>, ApiError>
...
async fn open_export_location<R>(
    State(state): State<HttpState<R>>,
    Path(job_id): Path<String>,
...
fn parse_export_id(value: String, code: &'static str, label: &str) -> Result<i64, ApiError> {
    value
        .parse::<i64>()
        .ok()
        .filter(|id| *id > 0)
...
fn export_job_response<R: RecycleBin>(
    application: &ApplicationService<R>,
    job_id: i64,
...
fn spawn_export_worker<R>(application: SharedApplication<R>, request: ExportExecutionRequest)
where
    R: RecycleBin + Send + 'static,
{
    thread::spawn(move || {
...
fn persist_completed_output<E, F>(output: &std::path::Path, complete: F) -> Result<(), String>
where
    E: fmt::Display,
    F: FnOnce() -> Result<(), E>,
{
