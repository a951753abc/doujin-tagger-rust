...
| 全部藏書 | 搜尋、篩選、排序、切換列表／比較模式，並選取收藏送往整理台。 |
| 整理台 | 集中所有待人工處理的佇列，以頁內分頁切換：待歸檔（下載區收藏的歸檔預檢）、品質審核（缺少 metadata 或候選待裁決）、重複作品（內容指紋與重複候選）、批次操作（批次補資料、加標籤、改名、搬移、匯出、刪除與身分裁決）、名稱治理（metadata 正式名稱與變體合併）。 |
| 外部書庫 | 翻查 ExHentai gallery 與可用 torrent。 |
| 統計 | 查看有效收藏的分類、場次、社團、作者、原作與標籤分布。 |
| 設定 | 管理閱讀器、縮圖、掃描來源、典藏庫與匯出目的地。 |

搜尋列在所有頁面常駐；在非藏書頁面送出搜尋會直接切換到「全部藏書」顯示結果。

### 整理與批次操作

在「全部藏書」勾選收藏後前往「整理台」的「批次操作」分頁，即可批次加入標籤、指定原作或分類、補齊缺漏欄位、套用安全改名、搬移到典藏庫，或複製原始 ZIP 建立匯出套件。

涉及檔案變更的操作會在執行前與執行時重新檢查來源、目標、名稱及衝突。刪除時建議優先移到資源回收桶；永久刪除需要再次確認。

### 重複作品

「整理台」的「重複作品」分頁會在背景建立內容指紋，並將候選分為：
